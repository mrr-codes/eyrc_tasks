...
# markers = detect_ArUco("test_image1.png")
# angle = Calculate_orientation_in_degree(markers)
# mark_ArUco("test_image1.png", markers, angle)
...
def mark_ArUco(img, Detected_ArUco_markers, ArUco_marker_angles):
    ...
    for _id in Detected_ArUco_markers.keys():
        ...
        cv2.line(img, (int(ctr[0]), int(ctr[1])),
                 (int(mid[0]), int(mid[1])), (255, 0, 0), 2)
        cv2.putText(img, str(_id), ((int(ctr[0])+8), int(
            ctr[1])), fontFace=cv2.FONT_HERSHEY_COMPLEX, thickness=2, fontScale=0.75, color=(0, 0, 255))
        samp = str(round(ArUco_marker_angles[_id]))
        cv2.putText(img, samp, ((int(ctr[0])-90), int(ctr[1])),
                    fontFace=cv2.FONT_HERSHEY_COMPLEX, thickness=2, fontScale=0.75, color=(0, 255, 0))
        cv2.putText(img, "TEAM ID ss_1403", (0, 15), fontFace=cv2.FONT_HERSHEY_COMPLEX,
                    thickness=2, fontScale=0.75, color=(105, 53, 0))
