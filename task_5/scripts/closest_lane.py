    ...
    * Example Call: Callback function'''
    def spawn_clbk(self, msg):
        ...
        if self.spawn_count %2 != 0: 
            self.list_drone_1.append(msg.data)
            self.list_drone_1.sort()
            #print('Sorted row list for drone 0 is:', self.list_drone_0)
        else: 
            self.list_drone_0.append(msg.data)
            self.list_drone_0.sort()
            #print('Sorted row list for drone 1 is:', self.list_drone_1)
        self.spawn_count += 1
    ...
    * Example Call: example_position= check_position_0'''
    ...
    while not rospy.is_shutdown():
        ...
        if i > 2 and (len(setpoints_0)-1):
            ...
            if current_truck_location == 2:
                if len(stateMt.list_drone_0)>0:
                    required_row = stateMt.list_drone_0.pop(0)
                    required_row_setpoint = stateMt.calculate_row_start(required_row, 0)
            ...
            elif current_truck_location ==1:
                if len(stateMt.list_drone_0)>0:
                        required_row = stateMt.list_drone_0.pop()
                        required_row_setpoint = stateMt.calculate_row_start(required_row, 0)
        ...
        if len(img_proc.Detected_ArUco_markers_0) > 0 and box_dropped == False:
            ...
            ofb_ctl.offboard_set_mode_0()
            #print('All setpoints are', setpoints_0)
            pos_0.pose.position.x = setpoints_0[i][0]
            pos_0.pose.position.y = setpoints_0[i][1]
            pos_0.pose.position.z = setpoints_0[i][2]
            ...
            if reached == True and flag1 == False:
                ...
                while x == 0:
                    if len(stateMt.list_drone_0)>0:
                        ...
                        required_row_setpoint = (stateMt.local_pos_0.z, stateMt.local_pos_0.z, stateMt.local_pos_0.z) 
                    setpoints_0.extend([required_row_setpoint,(0,4,4),(0,4,4)])
            ...
            if last_pixel_aruco_detected_x<200 and last_pixel_aruco_detected_y<200:
                f = -1
                g = 1
            elif last_pixel_aruco_detected_x<200 and last_pixel_aruco_detected_y>200:
                f = -1
                g = -1 
            elif last_pixel_aruco_detected_x>200 and last_pixel_aruco_detected_y<200:
                f = 1
            ...
            vel_0.twist.linear.x = f
            vel_0.twist.linear.y = g
            vel_0.twist.linear.z = 0
...
def drone_1():
    ...
    setpoints_1= [(0, 0, 4)]#setpoints list
    # Create empty message containers for position and velocity messages 
    pos_1 = PoseStamped()
    ...
    * Example Call: example_position= check_position_1'''
    ...
    while not rospy.is_shutdown():
        ...
        if i >= 2 and (len(setpoints_1)-1):
            ...
            previous_y_error = 0
            if current_truck_location == 2:
                if len(stateMt.list_drone_1)>0:
                    required_row = stateMt.list_drone_1.pop(0)
                    required_row_setpoint = stateMt.calculate_row_start(required_row, 1)


                else:
                    required_row_setpoint = (stateMt.local_pos_1.x, stateMt.local_pos_1.y, stateMt.local_pos_1.z)                     
            
            elif current_truck_location ==1:
                if len(stateMt.list_drone_1)>0:
                        required_row = stateMt.list_drone_1.pop()
                        required_row_setpoint = stateMt.calculate_row_start(required_row, 1)
                       
                else:
                    required_row_setpoint = (stateMt.local_pos_1.x, stateMt.local_pos_1.y, stateMt.local_pos_1.z) 

            setpoints_1.extend([(stateMt.local_pos_1.x, stateMt.local_pos_1.y,7), required_row_setpoint, required_row_setpoint, required_row_setpoint])


        # Setting necessary variables on reaching row search start
        # if  i>=2 and flag_flip_pos_vol == True : 
        if  (pos_1.pose.position.x == required_row_setpoint[0] and pos_1.pose.position.y==required_row_setpoint[1]) and reached==True : 
            #print(i)
            box_dropped = False
            m = -1
        ...
        if len(img_proc.Detected_ArUco_markers_1) > 0 and box_dropped == False:
            ...
            if(((200 - img_proc.position_aruco_x_1)**2 + (225-img_proc.position_aruco_y_1)**2)<= (img_proc.exo_rad_1)**2) and ((225 <img_proc.position_aruco_y_1-4) and (193<img_proc.position_aruco_x_1)):
                ...
                if stateMt.check_gripper_1 == 'True':
                    setpoints_1.clear()
                    i=0
                    print('d1 setpoints have been cleared and i has been set to 0')
                    
                    setpoint = (stateMt.local_pos_1.x,stateMt.local_pos_1.y, 4)
                    setpoints_1.insert(0, setpoint) #Inserting setpoint for drone to raise to a height after picking up the box
                    #i = i+1
            ...
            previous_y_error = img_proc.position_aruco_y_1 - (200 + 80/stateMt.local_pos_1.z)#Calculating the previous y error based on distance between aruco and drone,height
            last_pixel_aruco_detected_x = img_proc.position_aruco_x_1
            last_pixel_aruco_detected_y = img_proc.position_aruco_y_1
            ...
            if reached == True and flag1 == False:
                ...
                while x == 0:
                    if len(stateMt.list_drone_1)>0:
                        ...
                        required_row_setpoint = (stateMt.local_pos_1.z, stateMt.local_pos_1.z, stateMt.local_pos_1.z) 

                    setpoints_1.extend([required_row_setpoint,(0,4,4),(0,4,4)])
                    print('d1 after reaching goal setpoints are (happens only once)', setpoints_1)                   
                    rospy.sleep(1)
            ...
            if (pos_1.pose.position.x == truck_pts[0][0] and pos_1.pose.position.y==truck_pts[0][1]) and reached==True :   
                ofb_ctl.setAutoLandMode_1()
            ...
            vel_1.twist.linear.x = f
            vel_1.twist.linear.y = g
            vel_1.twist.linear.z = 0
