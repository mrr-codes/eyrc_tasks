...
def drone_0():
    ...
    while not rospy.is_shutdown():
        ...
        if len(img_proc.Detected_ArUco_markers_0) > 0 and box_dropped == False:
            ...
            vel_0.twist.linear.x = (((img_proc.position_aruco_x_0 - 200)*stateMt.local_pos_0.z)/550)
            vel_0.twist.linear.y = -((((img_proc.position_aruco_y_0 - (200 + 80/stateMt.local_pos_0.z))*stateMt.local_pos_0.z)/500) - (img_proc.position_aruco_y_0 - (200 + 80/stateMt.local_pos_0.z) - previous_y_error)/40)
            print('d0 Box detected, the x and y velocities are:', vel_0.twist.linear.x, vel_0.twist.linear.y)
            vel_0.twist.linear.z = 0
            ...
            if (img_proc.bcorner_0[0] < (200) < img_proc.tpctr[0]) and (img_proc.tpctr[1] < (225) < img_proc.bcorner_0[1]):
                ...
                vel_0.twist.linear.x = (position_for_landing_x_0 - stateMt.local_pos.x)                
                vel_0.twist.linear.y = (position_for_landing_y_0 - stateMt.local_pos.y)
                vel_0.twist.linear.z = (0.2 - stateMt.local_pos.z)/150
                ...
                if stateMt.local_pos.z < 0.25:

                    print('d0 Attempted to land c=', str(land_count))
                    #rospy.sleep(8)
                    print("d0 trying to grip the box")
            ...
            if flag1 == False and stateMt.local_pos_0.z > 2.5 and stateMt.check_gripper_0 == 'False':
                local_vel_pub_0.publish(vel_0)
                position_for_landing_x_0 = stateMt.local_pos.x
                position_for_landing_y_0 = stateMt.local_pos.y
